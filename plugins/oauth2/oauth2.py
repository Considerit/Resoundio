...
def google_oauth2_authorization(
    redirect_uri, client_id, client_secret, oauth_callback, scope="profile email openid"
):
...
def oauth2_authorization(
    ...
    if state.done and redirect_back_to.done:
        ...
        authorization_response = (
            f"{loc.protocol}//{loc.host}{loc.path}?{loc.query_args}"
        )
